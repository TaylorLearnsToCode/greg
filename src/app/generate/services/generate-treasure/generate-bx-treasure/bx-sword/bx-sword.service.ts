...
export class BxSwordService {
  ...
  /** Sword intellgence map */
  private readonly SWORD_INTELLIGENCE: Map<BoundedRange, Number> = new Map([
    [new BoundedRange({ low: 1,  high: 14}), 0],
    [new BoundedRange({ low: 15, high: 15}), 0],
    [new BoundedRange({ low: 16, high: 16}), 0],
    [new BoundedRange({ low: 17, high: 17}), 9],
    [new BoundedRange({ low: 18, high: 18}), 10],
    [new BoundedRange({ low: 18, high: 19}), 11],
    [new BoundedRange({ low: 20, high: 20}), 12],
  ])
  /** Alignment map for swords */
  private readonly SWORD_ALIGNMENTS: Map<BoundedRange, Alignment> = new Map([
    [new BoundedRange({ low: 1, high: 13 }), Alignment.LAW],
    [new BoundedRange({ low: 14, high: 18 }), Alignment.NEUTRALITY],
    [new BoundedRange({ low: 19, high: 20 }), Alignment.CHAOS],
  ]);
  ...
  private readonly SWORD_COMMUNICATION_MODES: Map<BoundedRange, string> =
    new Map([
      ...
      [new BoundedRange({ low: 10, high: 12 }), 'Speech'],
    ]);

  /** Can the sword read? */
  private readonly SWORD_READING: Map<BoundedRange, string> =
    new Map([
      [new BoundedRange({ low: 1, high: 10 }), 'No'],
      [new BoundedRange({ low: 11, high: 12 }), 'Yes'],
    ]);
  ...
  private readonly SWORD_SPECIAL_PURPOSES: string[] = [
    'Slay magic-users (includes elves)',
    'Slay clerics',
    'Slay Fighters',
    'Slay A specific monster',
    'Defeat Law (if sword is Lawful, defeat Chaos)',
    'Defeat Chaos(if sword is Chaotic, defeat Law)',
  ];
  ...
  appendSwordCharacteristics(name: string): string {
    ...
    if (newName.includes('Sword') && !newName.includes('Map to')) {
      this.isSpecialPurpose = rollDice(this.d20) == 20;
      this.swordIntelligence = this.isSpecialPurpose ? 12 : this.determineIntelligence();
  ...
  /** Determines a sword's intelligence via the B/X Algorithm (Note, the OSE method **IS** different, though the percantage chance is the same) */
  private determineIntelligence(): number {
    let roll = rollDice(this.d20)
    let swordIntelligence: number = 0
    for(const intelligence of this.SWORD_INTELLIGENCE.keys()) {
      if (isBetween(roll, intelligence)) {
        swordIntelligence = this.SWORD_INTELLIGENCE.get(intelligence) as number;
      }
    }
    return swordIntelligence;
  }
  /** Generates and returns the sword's alignment. All swords have an alignment. */
  private appendAlignment(): string {
    let alignment: string = '';
    const roll: number = rollDice(this.d20);
   ...
   * @todo: Possibly need to extend the ReferenceEntry  
   */
